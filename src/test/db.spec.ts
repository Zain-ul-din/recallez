...
describe("Neo4j Tests", () => {
  ...
  describe("Idempotent Queries Using User Id", () => {
    const userId = "LNSCRvfufEYUHxF+LhpDpisAxSIZyplp";

    it("should clear the database", async () => {
      const res = await session.run("MATCH (n) DETACH DELETE n RETURN n;");
      expect(res.records.length === 0);
    });

    it("should create single bookmark node", async () => {
      const query = `
        MERGE (s:Site { url: $url, userId: $userId }) 
        SET s = { name: $name, url: $url, userId: $userId }
        RETURN s;
      `;
    ...
    it("should create multiple bookmark nodes", async () => {
      const query = `
          UNWIND $sites AS site
          MERGE (s:Site { url: site.url, userId: site.userId })
          SET s = { name: site.name, url: site.url, userId: site.userId }
          RETURN s;
        `;

      const sites = [
        { name: "Github", url: "https://www.github.com", userId },
        { name: "Stack Overflow", url: "https://stackoverflow.com", userId },
        { name: "MDN", url: "https://developer.mozilla.org", userId }
      ];
    ...
    it("should update existing node with the same URL or create new one", async () => {
      const query = `
            MERGE (s:Site { url: $url, userId: $userId })
            SET s = { url: $url, name: $name, userId: $userId }
            RETURN s;
        `;
      ...
      const searchQuery = `
        MATCH (n:Site)
        WHERE n.url = $url AND n.userId = $userId
        RETURN n;
      `;
